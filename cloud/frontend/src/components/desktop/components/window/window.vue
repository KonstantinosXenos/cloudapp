...
<transition name="slide-fade">
  <vuedraggableresizable
    ...
    @click.native.stop='get_focus'
    @dblclick.native.stop='get_focus'
    :z-index="z_index"
  ><div :class="[{'radius':!maximized},'window']">
    <div class="drag-handle" @dblclick="maximize">
      <div class="title vertical-center" v-text="title"></div>
      <div class="button-container">
        ...
        <closeButton class="vertical-center" @click.native.stop="close_app" @mousedown.native.stop @dblclick.native.stop/>
      </div>
    </div>
...
export default {
  ...
  computed: {
    is_minimized() {
      console.log(this.data.unique_name)
      return this.$store.getters["taskManager/minimized_windows"].includes(this.data.unique_name);
    },
    z_index() {
      return this.$store.getters["taskManager/open_windows"].indexOf(this.data);
    },
    title() {
      return this.$store.getters["taskManager/open_windows"].filter(window => window.unique_name == this.data.unique_name)[0].title
    },
    maximized_height() {
  ...
  methods: {
    ...
    minimize() {
      this.$store.commit("taskManager/add_minimized", this.data.unique_name);
    }
